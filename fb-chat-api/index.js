...
function buildAPI(globalOptions, html, jar) {
	...
	if (maybeCookie.length === 0) {
		throw { error: "Bị die acc hay sao rồi kìa lồn ngu" };
	}

	if (html.indexOf("/checkpoint/block/?next") > -1) {
		log.warn("login", "bị checkpoint rồi kìa , mày vô acc để duyệt đi");
	}

	const userID = maybeCookie[0].cookieString().split("=")[1].toString();
	const i_userID = objCookie.i_user || null;
	log.info("Đăng Nhập", `Uid ${userID}`);
	...
	let mqttEndpoint, region, fb_dtsg, irisSeqID;
        try {
           ...
           if (endpointMatch) {
            ...
            log.info('Đăng Nhập', `Máy Chủ ${region}`);
         } catch (e) {
            log.warning('Đăng Nhập', 'Not MQTT endpoint');
         }
