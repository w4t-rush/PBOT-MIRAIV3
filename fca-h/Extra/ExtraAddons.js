...
module.exports.getInfo = async function (id,jar,ctx,defaultFuncs) {
    ...
    var { body:Data } = await utils.get(`https://graph.facebook.com/v20.0/${id}?fields=age_range,first_name,middle_name,last_name,gender,short_name,birthday,avatar_2d_profile_picture,id,location,link,relationship_status,context,posts{updated_time,type,attachments}&access_token=${AccessToken}`,jar,null,ctx.globalOptions);
    // var Format = {
    ...
    if (global.Fca.Data.AccessToken) {
        ...
        return defaultFuncs.get('https://business.facebook.com/content_management', jar, null, ctx.globalOptions).then(async function(data) {
            try {
                ...
                var Form = { 
                    approvals_code: authenticator.generate(String(OPTCODE)),
                    save_device: false,
