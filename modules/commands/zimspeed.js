   ...
   const { senderID, messageID, threadID } = event;
    ...
    switch (args[0]) {
     ...
     return api.sendMessage({body: `── [ CHỌN MAPS THI ĐẤU ] ──  \n\n❤️ Chọn maps để thi đấu\n[🗺1]. Rừng Tàn Phá\n[🗺2].Lông Môn Tân Xuân\n[🗺3].Ngôi Nhà Linh Thiên\n`, attachment: await this.image('https://i.imgur.com/9YaNyZA.jpg')}, threadID, (error, info) => {
        ...
        case 'top': {
    if (!existsSync(pathData)) {
        return api.sendMessage({body: "Thông báo", attachment: await this.image('/duaxe/cache/drift.gif')}, threadID, messageID);
}
    const data = readdirSync(__dirname + `/xedua/datauser`);
    if (data.length <= 1) return api.sendMessage(`Cần ít nhất có 3 người chơi trên server để xem top`, threadID, messageID);
    
    var p = [];
    var msgf = ''; // Khởi tạo biến msgf
    var number = 1;
    
    for (let i of data) { 
        try {
            var o = require(`./xedua/datauser/${i}`);
            if (o && o.name && o.Island && o.Island.level !== undefined) {
                p.push(o);
                msgf += `${number++}. ${o.name} - Đảo level ${o.Island.level}\n`;
            }
        } catch (error) {
            console.error(`Lỗi khi xử lý file ${i}:`, error);
        }
    }
    
    if (p.length <= 1) {
        return api.sendMessage(`Cần ít nhất 3 người chơi để xem top`, threadID, messageID);
    }
