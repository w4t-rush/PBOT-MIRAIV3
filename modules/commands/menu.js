module.exports.config = {
    ...
    cooldowns: 5,
    images: ['/cache/dos.gif'],  // Đặt ảnh cố định là dos.gif
    envConfig: {
        autoUnsend: {
            status: true,
            timeOut: 60
...
module.exports.run = async function ({ api, event, args }) {
    ...
    // Sử dụng ảnh đã chỉ định trong config
    const imgPath = '/cache/dos.gif';  // Đường dẫn ảnh đã được xác định
    ...
    if (args.length >= 1) {
        if (typeof cmds.get(args.join(' ')) == 'object') {
            ...
            if (args[0] == 'all') {
                ...
                return send({ body: txt, attachment: imgPath }, tid, (a, b) => autoUnsend.status ? setTimeout(v1 => un(v1), 1000 * autoUnsend.timeOut, b.messageID) : '');
            } else {
        ...
        return send({ body: txt, attachment: imgPath }, tid, (a, b) => {
...
module.exports.handleReply = async function ({ handleReply: $, api, event }) {
    ...
    const imgPath = '/cache/dos.gif';  // Đường dẫn ảnh
