...
try {
    ...
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/lekhanh/FastConfigFca.json')) {
        global.Fca.Require.fs.writeFileSync(process.cwd() + "/lekhanh/FastConfigFca.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));
        process.exit(1);
    }

try {
    var Data_Setting = require(process.cwd() + "/lekhanh/FastConfigFca.json");
}
catch (e) {
    ...
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/lekhanh/FastConfigFca.json')) {
        ...
        for (let i of All_Variable) {
            if (Data_Setting[i] == undefined) {
                ...
                global.Fca.Require.fs.writeFileSync(process.cwd() + "/lekhanh/FastConfigFca.json", JSON.stringify(Data_Setting, null, "\t"));
            }
            else continue; 
        } //Check Variable
            ...
            if (global.Fca.Require.FastConfig.Stable_Version.Accept == true) {
                if (Data.Stable_Version.Valid_Version.includes(global.Fca.Require.FastConfig.Stable_Version.Version)) {
                    ...
                    log.warn("[ FCA-UPDATE ] •", "Error Stable Version, Please Check Your Stable Version in FastConfig.json, Automatically turn off Stable Version!");
                        ...
                        global.Fca.Require.fs.writeFileSync(process.cwd() + "/lekhanh/FastConfigFca.json", JSON.stringify(global.Fca.Require.FastConfig, null, "\t"));
                    process.exit(1);
                }
            }
            else {
